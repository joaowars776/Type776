...
import { UserProfile, INITIAL_PROFILE, GameMode, TestResult, Theme, AppFont, HabboGameType, DiversosResult, HabboDifficulty, SoundType } from './types';
...
function App() {
  // Initialize state synchronously from localStorage to avoid "Loading..." flash
  const [profile, setProfile] = useState<UserProfile>(() => loadProfile());
  const [currentView, setCurrentView] = useState<'game' | 'profile' | 'habbo'>('game');
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);
  const settingsRef = useRef<HTMLDivElement>(null);
  
  const [previousFont, setPreviousFont] = useState<AppFont>(profile.settings.font);
  ...
  const [, setShowAchievementList] = useState(false);
  ...
  useEffect(() => {
    // Save profile whenever it changes (debouncing could be added if needed)
  ...
  const handleTestComplete = (result: TestResult) => {
    ...
    if (missionJustFinished) {
        // We don't set showResult(null) here because we might want to see results even if a mission finished
        // But the original code had logic to show mission completion instead of result
        // Let's keep it similar but for multiple
    }
  ...
  const startMission = (achId: string) => {
      if (activeMissionIds.length >= 3) return;
      if (activeMissionIds.includes(achId)) return;
      setActiveMissionIds(prev => [...prev, achId]);
  };

  const cancelMission = (id: string) => {
      setActiveMissionIds(prev => prev.filter(m => m !== id));
      setCompletedMissionIds(prev => prev.filter(m => m !== id));
  };

  const returnToMissionSelect = (id: string) => {
      setActiveMissionIds(prev => prev.filter(m => m !== id));
      setCompletedMissionIds(prev => prev.filter(m => m !== id));
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-500 ${themeStyles.bg} ${themeStyles.text} ${profile.settings.font !== AppFont.INTER ? '' : 'font-sans'} selection:bg-blue-500/30 overflow-x-hidden`} style={fontStyle}>
        ...
        <MissionTracker 
          ...
          onReturnToMissions={returnToMissionSelect} 

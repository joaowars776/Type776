...
const HabboGame: React.FC<Props> = ({ type, settings, onBack, onComplete, sessionScore, randomConfig, sessionStats }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full max-w-4xl mx-auto min-h-[50vh] animate-fade-in-up relative">
            ...
            <div className="mb-8 text-center flex flex-col gap-2 items-center">
                ...
                {boostActive && (
                    <div className="flex flex-col items-center gap-1">
                        <span className="text-sm font-black text-yellow-300 bg-yellow-900/30 px-4 py-1 rounded-full border border-yellow-500/50 shadow-[0_0_10px_rgba(234,179,8,0.3)] tracking-wide animate-pulse flex items-center gap-2">
                            {tr('boost_hint_active')} ({tr(difficulty)})
                        </span>
                        <div className="flex gap-1">
                            {Array.from({ length: difficulty === HabboDifficulty.EASY ? 1 : difficulty === HabboDifficulty.STANDARD ? 2 : 3 }).map((_, i) => (
                                <Zap key={i} size={16} className="fill-yellow-400 text-yellow-400 drop-shadow-[0_0_5px_rgba(234,179,8,0.8)]" />
                            ))}
                        </div>
                    </div>
                )}
            ...
            <div className={`w-full ${isRandomMode ? 'bg-black/40 backdrop-blur-md border border-white/10' : themeStyles.panel} rounded-2xl p-8 sm:p-12 shadow-2xl relative overflow-hidden transition-colors duration-300`}>
                {isCurrentRepita ? (

...
interface Props {
  ...
  onMissionCancel?: (id: string) => void;
  isInputModalOpen?: boolean;
}

const TypingTest: React.FC<Props> = ({ 
    settings, mode, onComplete, onUpdateSettings, stats, 
    onModeChange, achievements = [], onMissionStart, activeMissionIds = [], 
          ...
          <div className="w-full max-w-5xl mx-auto flex flex-col gap-6 animate-fade-in-up">
              ...
              <div className={`flex flex-wrap gap-4 justify-between items-center p-4 rounded-xl border ${isLight ? 'bg-white/80 border-gray-300 shadow-md' : 'bg-black/20 border-gray-500/10'}`}>
                  <div className="flex gap-2 flex-wrap">
                      <button onClick={() => setMissionDifficulty('all')} className={`px-3 py-1 rounded text-sm border ${missionDifficulty === 'all' ? 'bg-blue-500 text-white border-blue-600' : 'border-transparent bg-black/10'}`}>{t('filter_all')}</button>
                  ...
                  <div className="flex gap-2">
                      <button onClick={() => setMissionSort('xp_desc')} className={`px-3 py-1 rounded text-sm border ${missionSort === 'xp_desc' ? 'bg-purple-500 text-white border-purple-600' : 'border-transparent bg-black/10'}`}>{t('sort_xp_desc')}</button>
                      <button onClick={() => setMissionSort('xp_asc')} className={`px-3 py-1 rounded text-sm border ${missionSort === 'xp_asc' ? 'bg-purple-500 text-white border-purple-600' : 'border-transparent bg-black/10'}`}>{t('sort_xp_asc')}</button>
                  </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
                  {filteredMissions.map(ach => {
                      const isActive = activeMissionIds.includes(ach.id);
                      const canStart = activeMissionIds.length < 3;
                      ...
                      return (
                          <div key={ach.id} className={`${themeConfig.panel} p-4 rounded-xl border border-gray-500/10 flex flex-col justify-between relative overflow-hidden group`}>
                              {isActive && <div className="absolute top-0 right-0 p-1 bg-green-500/20 rounded-bl text-[10px] text-green-400 font-bold uppercase">{t('mission_active_label')}</div>}
                              ...
                              {isActive ? (
                                  <button onClick={() => onMissionCancel && onMissionCancel(ach.id)} className="w-full py-2 bg-red-600 hover:bg-red-500 text-white rounded text-sm font-bold mt-2 transition-colors shadow-lg">{t('cancel_mission')}</button>
                              ) : (
                                  <button 
                                    onClick={() => onMissionStart && onMissionStart(ach.id)} 
                                    disabled={!canStart}
                                    className={`w-full py-2 rounded text-sm font-bold mt-2 transition-colors border ${canStart ? 'bg-blue-600/20 hover:bg-blue-600/30 text-blue-500 border-blue-500/30' : 'bg-gray-500/10 text-gray-500 border-gray-500/20 cursor-not-allowed'}`}
                                  >
                                    {t('start_mission')}
                                  </button>
                              )}
